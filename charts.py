...
def return_temporal_map(data_subset, data_full):
    chart_base_map = alt.Chart(source
            ...
            lookup='id',
            from_=alt.LookupData(data_subset, 'country-code', ['Country','Year','Cases','cases_bucket']),
        )
    ...
    data_full['cases_bucket'] = pd.cut(data_full['Cases'], bins=bins, labels=labels)
...
def return_art_map(data_subset, data_full):
    ...
    rate_color = alt.Color(field="Proportion of HIV patients (%)", type="category", scale=rate_scale)
